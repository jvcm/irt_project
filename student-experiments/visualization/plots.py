...
def plot_noisy_points(xtest, disc=None):
    ...
    lgd.append('non-noise data point')
    plt.scatter(xtest.x[xtest.noise>0],xtest.y[xtest.noise>0],c=cls[3],s=60)
    lgd.append('noise data point')
    if not disc is None:
        plt.scatter(xtest.x[disc<0],xtest.y[disc<0],c=cls[0],marker='+',facecolors='none')
        lgd.append('detected noise data point')
    
    plt.title('True and detected noise data points')
    l = plt.legend(lgd,frameon=True)
